...
int main()
{
	...
	while (option != 0)
	{
		...
		while (option == -1)
		{
			...
			std::cout<<"7. Toggle Multi vs. Single hreads (Current="<<threadCount<<")"<< std::endl;
		...
		case 7: threadCount = (1*(threadCount != 1)) +
		...
		// GOPS is billion operations per second
		...
		std::cout<<"Score: "<< (gops / phenom2) << " Phenoms II's worth" << std::endl;
	}
